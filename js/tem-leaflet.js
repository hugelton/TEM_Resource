...
(function() {
    ...
    function initMap() {
        ...
        if (typeof L !== 'undefined') {
            ...
            try {
                ...
                marker.bindPopup(`<b>${currentData.cityName || currentData.city || 'Current Location'}</b><br>${currentData.latitude?.toFixed(4) || 35.6762}, ${currentData.longitude?.toFixed(4) || 139.6503}`).openPopup();
                ...
                map.on('click', function(e) {
                    ...
                    marker.setPopupContent(`<b>New Location</b><br>${lat.toFixed(4)}, ${lng.toFixed(4)}`).openPopup();
                ...
                window.updateMapLocation = function() {
                    if (window.temMap && window.temMarker) {
                        const newLatLng = [currentData.latitude, currentData.longitude];
                        window.temMarker.setLatLng(newLatLng);
                        window.temMap.setView(newLatLng, window.temMap.getZoom());
                        window.temMarker.setPopupContent(`<b>${currentData.cityName || currentData.city || 'Location'}</b><br>${currentData.latitude.toFixed(4)}, ${currentData.longitude.toFixed(4)}`);
                    }
                };
            ...
            // Fallback map without Leaflet
            initFallbackMap(mapEl);
        }
    }
    
    // Fallback map for when Leaflet is not available
        ...
        mapEl.innerHTML = `
            <div style="height: 200px; background: linear-gradient(135deg, #1a1a1a 0%, #2d2d2d 100%); 
                ...
                <div style="text-align: center; color: #888;">
                    ...
                    <div style="font-size: 13px; opacity: 0.7;">${currentData.latitude?.toFixed(3) || 0}°, ${currentData.longitude?.toFixed(3) || 0}°</div>
    ...
    function updateUI() {
        ...
        // Update map location if it exists
